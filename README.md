...
| 12  | [Field Metadata](./apps/12-field-metadata)                 | Attach reactive metadata with `metadata()` / `createMetadataKey`, read the keys `required`/`maxLength` publish, and run a managed `httpResource` preview |
